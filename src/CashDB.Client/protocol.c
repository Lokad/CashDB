...
// Open Block
cashdb_status_enum_t open_block(
	connection_s* conn,
...
// Commit Block
cashdb_status_enum_t commit_block(connection_s* conn, block_handle_t block, block_id_t* blockid)
{
	range buffer = connection_get_send_buffer(conn);
...
cashdb_status_enum_t get_committed_block_handle(connection_s* conn, block_id_t* blockid, block_handle_t* result)
{
	range buffer = connection_get_send_buffer(conn);
...
cashdb_status_enum_t get_uncommitted_block_handle(
	connection_s* conn,
	block_ucid_t* block_ucid,
...
// Get Block Info
cashdb_status_enum_t get_block_info(connection_s* conn, block_handle_t block, block_info_t* info)
{
	range buffer = connection_get_send_buffer(conn);
	...
	info->flags = response.isCommitted == 1 ? CASHDB_BLOCK_COMMITTED : 0;
...
// Set Coins
cashdb_status_enum_t set_coins(
	connection_s* conn, 
	...
	uint8_t* storage)
{
	...
	for (coin_t* coin = coins; coin < end; coin++)
	{
		...
		if (coin->script_offset < 0)
			return CASHDB_ERR_INVALID_REQUEST;
	...
	for (int32_t i = 0; i < coin_length; i++)
	{
		...
		{
		case ccs_success:
			coin->status = CASHDB_COIN_STATUS_SUCCESS;
			break;
		case ccs_outpoint_not_found:
			coin->status = CASHDB_COIN_STATUS_OUTPOINT_NOT_FOUND;
			break;
		case ccs_invalid_context:
			coin->status = CASHDB_COIN_STATUS_INVALID_CONTEXT;
			break;
		case ccs_invalid_block_handle:
			coin->status = CASHDB_COIN_STATUS_INVALID_BLOCK_HANDLE;
			break;
		default:
			return TSE_INTERNAL_ERROR;
		}
...
// Get Coins
cashdb_status_enum_t get_coins(
	connection_s* conn,
	block_handle_t context,
	int32_t coin_length,
	coin_t* coins,
...
)
{
	...
	for (int32_t i = 0; i < coin_length; i++)
	{
		...
		{
			coin->status |= CASHDB_COIN_STATUS_STORAGE_TOO_SHORT;
		}
