/* CashDB Client-Server protocol

  This file implements the client-server protocol of CashDB which operates
  over a simple TCP socket. Unlike the primary C/C++ API of CashDB, backward 
...
/* Typed version of return codes in the CashDB API.
...
typedef enum
{
	TSE_SUCCESS = CASHDB_SUCCESS,
#define TSE_(x) TSE_ ## x = CASHDB_ERR_ ## x
	TSE_(CONNECTION_FAILED),
	/* the above expands to
		  TSE_CONNECTION_FAILED = CASHDB_ERR_CONNECTION_FAILED
...
#undef TSE_
} cashdb_status_enum_t;


cashdb_status_enum_t open_block(connection_s* conn, 
	block_id_t* parent_id, block_handle_t* block, block_ucid_t* block_ucid);

cashdb_status_enum_t commit_block(connection_s* conn, 
	block_handle_t block, block_id_t* blockid);

cashdb_status_enum_t get_committed_block_handle(connection_s* conn, 
	block_id_t* blockid, block_handle_t* result);

cashdb_status_enum_t get_uncommitted_block_handle(connection_s* conn, 
	block_ucid_t* block_ucid, block_handle_t* result);

cashdb_status_enum_t get_block_info(connection_s* conn, 
