...
{
    /// <summary>
    /// Permits reading out a well-formed xml document representing a CashDB
    ...
    /// </summary>
    [DataContract(Namespace = "", Name = "CashDBConfig")]
    public class CashDBConfig
    {
    ...
    public static class CashDBConfigReader
    {
        public static CashDBConfig Read(string configPath)
        {
            using (var configReader = new FileStream(configPath, FileMode.Open))
            {
                var ser = new DataContractSerializer(typeof(CashDBConfig));
                var config = (CashDBConfig) ser.ReadObject(configReader);
        ...
        private static void ValidateConfig(CashDBConfig config)
        {
