...
{
    public unsafe ref struct RemoveCoinResponse
    {
            ...
            SpanPool<byte> pool)
        {
            ...
            AsHeader.ResponseHeader.MessageKind = MessageKind.RemoveCoinResponse;

            AsHeader.Status = status;
        }

        public RemoveCoinResponse(Span<byte> span)
        {
            _buffer = span;
        }
